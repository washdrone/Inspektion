...
const PROD_ORIGIN = 'https://www.surveydrone.se'
...
// 6. Hreflang i sitemap: ömsesidiga sv/en-par
{
  const blocks = [...xml.matchAll(/<url>([\s\S]*?)<\/url>/g)].map((m) => m[1])
  let withAlt = 0, broken = 0
  const locOf = (b) => (b.match(/<loc>(.*?)<\/loc>/) || [])[1]
  const map = new Map(blocks.map((b) => [locOf(b), b]))
  for (const b of blocks) {
    const alts = [...b.matchAll(/hreflang="(sv|en)"[^>]*href="([^"]*)"/g)]
    if (!alts.length) continue
    withAlt++
    for (const [, , href] of alts) {
      const other = map.get(href)
      if (!other) { broken++; fail(`hreflang pekar på URL utanför sitemap: ${href}`); continue }
      if (!other.includes(`href="${locOf(b)}"`)) { broken++; fail(`hreflang ej ömsesidig: ${locOf(b)} <-> ${href}`) }
    }
  }
