  ...
  if (formState === 'success') {
    return (
      <div role="status" aria-live="polite" className="rounded-2xl border border-brand-200 bg-brand-50 p-8 text-center">
        ...
        <p className="mt-2 text-sm text-dark-500">We will get back to you as soon as we can with questions, a quote or a suggested next step.</p>
      </div>
    )
  }
      ...
      <form onSubmit={handleSubmit} className="mt-6 space-y-5" aria-busy={formState === 'submitting'}>
        ...
        <div className="grid gap-5 sm:grid-cols-2">
          <div>
            ...
            <input type="text" id="name" name="name" autoComplete="name" maxLength={120} required onFocus={handleFocus} className={inputClasses} placeholder="Your name" />
          </div>
          <div>
            <label htmlFor="company" className="block text-sm font-medium text-dark-700">Company / organisation <span className="text-red-500" aria-label="required">*</span></label>
            <input type="text" id="company" name="company" autoComplete="organization" maxLength={200} required onFocus={handleFocus} className={inputClasses} placeholder="Company name" />
          </div>
        </div>
        <div>
          ...
          <input type="email" id="email" name="email" autoComplete="email" maxLength={254} required onFocus={handleFocus} className={inputClasses} placeholder="name@company.com" />
        </div>
        <div>
          <label htmlFor="service" className="block text-sm font-medium text-dark-700">Service</label>
          <select id="service" name="service" defaultValue={validService(serviceContext) ? serviceContext : ''} onFocus={handleFocus} className={inputClasses}>
            ...
            {CONTACT_SERVICES.map(item => <option key={item.en} value={item.en}>{item.labelEn}</option>)}
          </select>
        </div>
        <div>
          <label htmlFor="message" className="block text-sm font-medium text-dark-700">Describe Your Needs</label>
          <textarea id="message" name="message" maxLength={5000} rows={5} onFocus={handleFocus} className={inputClasses} placeholder="Describe the site, location and what you need to find out. Include approximate size and preferred timing if known." />
        </div>
        {formState === 'error' && (
          <div className="rounded-xl bg-red-50 p-4 text-sm text-red-700" role="alert">Something went wrong. Please try again or <a href="mailto:info@surveydrone.se" className="underline">email info@surveydrone.se</a>.</div>
        )}
        ...
        <p className="text-center text-sm text-dark-600">We use these details to respond to your enquiry. Please do not include sensitive information.</p>
      </form>
    </div>
  )
}
